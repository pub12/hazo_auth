...
export async function POST(request) {
    ...
    try {
        ...
        if (!org_structure || !org_structure.trim()) {
            return NextResponse.json({ error: "org_structure is required" }, { status: 400 });
        }
        // Validate firm_name length
        if (firm_name.length > 100) {
            return NextResponse.json({ error: "firm_name must be 100 characters or less" }, { status: 400 });
        }
        // Validate org_structure length
        if (org_structure.length > 50) {
            return NextResponse.json({ error: "org_structure must be 50 characters or less" }, { status: 400 });
        }
        const hazoConnect = get_hazo_connect_instance();
        // Check if user already has a scope assignment
        const has_scope = await user_has_any_scope(hazoConnect, auth.user.id);
        if (has_scope) {
            return NextResponse.json({ error: "User already belongs to a firm" }, { status: 400 });
        }
        // Create the firm
        ...
        if (!result.success) {
            ...
            return NextResponse.json({ error: result.error || "Failed to create firm" }, { status: 500 });
        }
        logger.info("firm_created", {
            filename: get_filename(),
            line_number: get_line_number(),
            user_id: auth.user.id,
            scope_id: (_a = result.scope) === null || _a === void 0 ? void 0 : _a.id,
            firm_name,
        });
