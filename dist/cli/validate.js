...
function check_schema(project_root) {
    var _a, _b;
    const results = [];
    const hazo_config_path = path.join(project_root, "config", "hazo_auth_config.ini");
    const hazo_config = read_ini_file(hazo_config_path);
    ...
    try {
        ...
        if (existing_tables.includes("hazo_user_scopes")) {
            const us_columns = db.prepare("PRAGMA table_info(hazo_user_scopes)").all();
            const col_names = us_columns.map((c) => c.name);
        ...
        if (existing_tables.includes("hazo_permissions")) {
            const perms = db.prepare("SELECT permission_name FROM hazo_permissions").all();
            const perm_names = perms.map((p) => p.permission_name);
