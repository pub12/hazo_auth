 ...
 */
export function sanitize_error_for_user(error, options = {}) {
    const { logToConsole = true, logToLogger = true, logger, context = {} } = options;
    // Extract detailed error message
    const detailed_error_message = error instanceof Error ? error.message : String(error);
    const error_stack = error instanceof Error ? error.stack : undefined;
    // Log detailed error to console
    if (logToConsole) {
        console.error("Detailed error:", Object.assign({ message: detailed_error_message, stack: error_stack }, context));
    }
    // Log detailed error to logger if provided
    if (logToLogger && logger) {
        logger.error("error_occurred", Object.assign({ filename: context.filename || "unknown", line_number: context.line_number || 0, error_message: detailed_error_message, error_stack }, context));
    }
    // Check if error is a PostgREST or database-related error
    const is_database_error = detailed_error_message.includes("PostgREST") ||
