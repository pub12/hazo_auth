...
import { DEFAULT_SYSTEM_SCOPE_ID } from "../../lib/services/scope_service";
  ...
  try {
    ...
    if (Array.isArray(existing_assignments) && existing_assignments.length > 0) {
      ...
      if (existing.role_id === role_id_str) {
        return NextResponse.json(
          { error: "Role is already assigned to this user in this scope" },
          { status: 409 }
        );
      }
      ...
      return NextResponse.json(
        { error: "User already has a role in this scope. Use PUT to update." },
        { status: 409 }
      );
    }
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(current_user_scopes)) {
      return NextResponse.json(
        { error: "Failed to fetch current user scopes" },
        { status: 500 }
      );
    }
    ...
    const current_role_ids_set = new Set<string>();
    ...
    if (to_remove.length > 0) {
      try {
        ...
        for (const role_id of to_remove) {
          ...
          const scope_assignments = current_user_scopes.filter(
            (us) => us.role_id === role_id
          );
