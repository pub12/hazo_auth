...
import {
  create_invitation,
  list_invitations_by_scope,
  list_all_invitations,
  revoke_invitation,
  get_invitation_by_id,
  type InvitationStatus,
} from "../../../../lib/services/invitation_service";
import {
  is_user_super_admin,
  get_user_scopes,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const status = status_param as InvitationStatus | undefined;
    ...
    if (is_super) {
      ...
      if (!scope_id) {
        ...
        const all_invitations: unknown[] = [];
        for (const scope of user_scopes.scopes) {
          const scope_result = await list_invitations_by_scope(
            hazoConnect,
            scope.scope_id,
            status
          );
        ...
        return NextResponse.json(
          { success: true, invitations: all_invitations },
          { status: 200 }
        );
      }
      ...
      const user_scopes = await get_user_scopes(hazoConnect, auth.user.id);
      const has_scope_access = user_scopes.scopes?.some(
        (s) => s.scope_id === scope_id
      );

      if (!has_scope_access) {
        return NextResponse.json(
          { error: "Access denied to this scope" },
          { status: 403 }
        );
      }
    ...
    logger.info("invitations_list_fetched", {
      ...
      scope_id,
      count: result.invitations?.length || 0,
    });

    return NextResponse.json(
      { success: true, invitations: result.invitations || [] },
      { status: 200 }
    );
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!email_address || !scope_id || !role_id) {
      return NextResponse.json(
        { error: "email_address, scope_id, and role_id are required" },
        { status: 400 }
      );
    }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (!is_super) {
      const user_scopes = await get_user_scopes(hazoConnect, auth.user.id);
      const has_scope_access = user_scopes.scopes?.some(
        (s) => s.scope_id === invitation_result.invitation?.scope_id
      );
