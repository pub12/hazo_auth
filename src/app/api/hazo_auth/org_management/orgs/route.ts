  ...
  try {
    ...
    if (!is_multi_tenancy_enabled()) {
      return NextResponse.json(
        { error: "Multi-tenancy is not enabled", code: "MULTI_TENANCY_DISABLED" },
        { status: 400 }
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!perm_check.is_global_admin && parent_org_id) {
      const access_check = await check_user_org_access(
        adapter,
        perm_check.user_org_id || null,
        perm_check.user_root_org_id || null,
        parent_org_id
      );

      if (!access_check.has_access) {
        return NextResponse.json(
          { error: "You do not have access to the parent organization" },
          { status: 403 }
        );
      }
    }
    ...
    if (!perm_check.is_global_admin && !parent_org_id) {
      return NextResponse.json(
        { error: "Only global admins can create root organizations" },
        { status: 403 }
      );
    }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    // Build update data
    const update_data: { name?: string; user_limit?: number; changed_by: string } = {
    ...
    if (user_limit !== undefined) {
      if (typeof user_limit !== "number" || user_limit < 0) {
        return NextResponse.json(
          { error: "user_limit must be a non-negative number" },
          { status: 400 }
        );
      }
