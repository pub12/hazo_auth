// file_description: service for creating new firms (root scopes) for users
// section: imports
import type { HazoConnectAdapter } from "hazo_connect";
import { createCrudService } from "hazo_connect/server";
import { create_app_logger } from "../app_logger";
import { sanitize_error_for_user } from "../utils/error_sanitizer";
import { create_scope, type ScopeRecord } from "./scope_service";
import { assign_user_scope, type UserScope } from "./user_scope_service";

// section: types

export type CreateFirmData = {
  firm_name: string;
  org_structure: string; // e.g., "Headquarters", "Head Office"
  user_id: string;
  role_id?: string; // Optional - will use firm_admin role if not provided
};
 ...
 */
export async function ensure_owner_role(
  adapter: HazoConnectAdapter,
): Promise<{ success: boolean; role_id?: string; error?: string }> {
  try {
    ...
    if (Array.isArray(existing_by_name) && existing_by_name.length > 0) {
      return {
        success: true,
        role_id: (existing_by_name[0] as { id: string }).id,
      };
    }
 ...
 */
async function assign_owner_permissions(
  adapter: HazoConnectAdapter,
  role_id: string,
): Promise<void> {
  try {
    ...
    for (const perm_name of permission_names) {
      ...
      if (!Array.isArray(permissions) || permissions.length === 0) {
        ...
        if (Array.isArray(inserted) && inserted.length > 0) {
          permission_id = (inserted[0] as { id: string }).id;
        } else {
          // Use the ID we generated even if insert didn't return it
          permission_id = perm_id;
        }
      } else {
        permission_id = (permissions[0] as { id: string }).id;
      }
 ...
 */
export async function create_firm(
  adapter: HazoConnectAdapter,
  data: CreateFirmData,
 ...
 * Gets a role by name
 */
export async function get_role_by_name(
  adapter: HazoConnectAdapter,
  role_name: string,
): Promise<{ success: boolean; role_id?: string; error?: string }> {
  try {
